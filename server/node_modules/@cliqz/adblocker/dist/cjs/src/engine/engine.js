...
const preprocessor_2 = require("./bucket/preprocessor");
exports.ENGINE_VERSION = 653;
...
class FilterEngine extends events_1.EventEmitter {
    ...
    static deserialize(serialized) {
        ...
        engine.preprocessors = preprocessor_2.default.deserialize(buffer);
        // Deserialize buckets
    ...
    cosmeticFilters = [], networkFilters = [], preprocessors = [], config = new config_1.default(), lists = new Map(), } = {}) {
        super(); // init super-class EventEmitter
        this.config = new config_1.default(config);
        // Subscription management: disabled by default
        this.lists = lists;
        // Preprocessors
        this.preprocessors = new preprocessor_2.default({});
        // $csp=
     ...
     */
    update({ newNetworkFilters = [], newCosmeticFilters = [], newPreprocessors = [], removedCosmeticFilters = [], removedNetworkFilters = [], removedPreprocessors = [], }, env = new preprocessor_1.Env()) {
        let updated = false;
        ...
        if (preprocessors !== undefined) {
            for (const [condition, details] of Object.entries(preprocessors)) {
                if (details.removed !== undefined && details.removed.length !== 0) {
                    const { networkFilters, cosmeticFilters } = (0, lists_1.parseFilters)(details.removed.join('\n'), this.config);
                    const filterIDs = new Set([]
                        .concat(cosmeticFilters.map((filter) => filter.getId()))
                        .concat(networkFilters.map((filter) => filter.getId())));
                    Array.prototype.push.apply(removedCosmeticFilters, cosmeticFilters);
                    Array.prototype.push.apply(removedNetworkFilters, networkFilters);
                    removedPreprocessors.push(new preprocessor_1.default({
                        condition,
                        filterIDs,
                    }));
                }
                if (details.added !== undefined && details.added.length !== 0) {
                    const { networkFilters, cosmeticFilters } = (0, lists_1.parseFilters)(details.added.join('\n'), this.config);
                    const filterIDs = new Set([]
                        .concat(cosmeticFilters.map((filter) => filter.getId()))
                        .concat(networkFilters.map((filter) => filter.getId())));
                    Array.prototype.push.apply(newCosmeticFilters, cosmeticFilters);
                    Array.prototype.push.apply(newNetworkFilters, networkFilters);
                    newPreprocessors.push(new preprocessor_1.default({
                        condition,
                        filterIDs,
                    }));
                }
